...
public class RoadGenScript : MonoBehaviour {
	...
	void ClickBtn() {
		GameObject.Find ("Car").transform.position = new Vector3 (START_POS_X, 0.5F, START_POS_Z + 1.25F);
	...
	int RollBack(GameObject road, int RollBackCount) {
		int RoadCount = 0;
		Vector2 rand = myRand (getRoadNum (road));
		for (int i = 1; i <= RollBackCount; i++) {
			RoadCount += (int)rand.y;
			RandList [getRandIndex ((getRoadNum (road) - RoadCount))].Set ((rand.x == 2 ? 0 : rand.x + 1), RandList [getRandIndex ((getRoadNum (road) - RoadCount))].y);
			rand = myRand (getRoadNum (road) - RoadCount);
		}

		updateRand (getRoadNum (road) - RoadCount + 1);
	...
	void updateRand(int roadCount) {
		List<Vector2> NewRandList = new List<Vector2> ();
		int direction, count;
		for (int i = 0; i < getRandIndex (roadCount); i++) {
			NewRandList.Add (RandList [i]);
		}
		for (int i = roadCount; i < ROAD_COUNT;) {
			direction = Random.Range (0, 8);
			count = Random.Range (1, 20);
			if (direction < 6) {
				i += count;
